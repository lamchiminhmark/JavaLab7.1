...
implements MapADT<K,V> {
    ...
    public V get(K searchKey) {
    	...
    	int compare = searchKey.compareTo(key);
    	if (compare < -1) return left.get(searchKey);
    ...
    protected V put(K key, V value, RBTreeMap<K,V> parent, RBTreeMap<K,V> gran){
	   ...
	   // TODO:  implement the other put cases
          ...
          if(comp==0){  
        	  rvalue = this.value;
        	  this.value = value;// base case II:  key is already present
          }
          else if(comp<0){ rvalue = left.put(key, value, this, parent);
          // recursive case I:  put in left subtree
	  }
	  else {  rvalue = right.put(key,  value, this, parent);
			  // recursive case II: put in right subtree
	  }
       }
